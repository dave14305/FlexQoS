...
write_iptables_rules() {
	...
	IFS="$OLDIFS"
	# [ -f "/tmp/${SCRIPTNAME}_iprules" ] && . /tmp/${SCRIPTNAME}_iprules
} # write_iptables_rules
...
generate_bwdpi_arrays() {
	...
	if ! [ -f /www/user/ext/${SCRIPTNAME}_arrays.js ] || [ /tmp/bwdpi.app.db -nt /www/user/ext/${SCRIPTNAME}_arrays.js ]; then
	{
